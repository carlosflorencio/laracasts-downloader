...
- **`App\Http\Resolver`** — all Laracasts HTTP traffic, sharing one Guzzle `CookieJar`. Login is a CSRF flow (fetch homepage for `XSRF-TOKEN` cookie, POST to `/sessions`). `downloadEpisode()` branches on `DOWNLOAD_SOURCE` env: `laracasts` follows the page's download link redirect (30/day limit, fixed quality); `mux` (default) re-fetches the episode page for a fresh signed playback token (~2h TTL, so tokens from scrape time would expire mid-sync) and delegates to `MuxDownloader`; `vimeo` delegates to `VimeoDownloader` (legacy — Laracasts no longer exposes Vimeo ids).
