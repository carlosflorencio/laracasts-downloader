...
# Only fetch .chapters.txt sidecars and/or .srt subtitles, or fix mtimes to the
# lesson publish dates (no videos); combinable. No longer require -s: with no
# filter they cover your whole downloaded library — the series list is scraped
# fresh (or read from cache.json with --cache-only), then restricted to series
# you actually have on disk. Always sidecar-only (chapters -> chapters/,
# subtitles -> subs/, never embedded, regardless of DOWNLOAD_CHAPTERS/
# DOWNLOAD_SUBTITLES). --timestamps-only stamps every downloaded video — pair
# with --cache-only to skip the scrape.
php start.php --timestamps-only                          # whole library, fresh list
php start.php --timestamps-only --cache-only             # whole library, from cache.json
php start.php --subtitles-only --chapters-only           # sidecars for every downloaded series
...
- **`App\Utils\Subtitles`** — subtitle delivery, mirroring `ChapterMetadata`. `DOWNLOAD_SUBTITLES` modes: `embed`/`true` muxes the `.vtt` tracks into the mp4 as `mov_text` streams (a stream-copy remux that carries chapters over, tags `language` via an ISO 639‑1→2 map, names each track's `handler_name` with the full language (e.g. "English") via `languageName()` — ICU/`ext-intl` `Locale::getDisplayLanguage`, so any code resolves without a hand-maintained list, falling back to the bare code; note MP4 has no per-stream `title` atom so `title=` would be silently dropped — sets the default track's disposition, and is `!`/`%`-safe + mtime-preserving like `Metadata::write`), `sidecar`/`file` saves them as `.srt` in a `subs/` subfolder (ffmpeg-converted from the source WebVTT via `vttToSrt()` — players such as VLC language-label external `.srt` sidecars by filename, never `.vtt`; the converter routes through `!`/`%`-safe sibling temp names + `rename`), `both` does both, `off`/`false`/`none` disables; **unset/empty (or any unrecognised value) defaults to `sidecar`** (`mode()`). Each downloader builds a track list, narrows it with `SubtitleLanguages::filter()`, materialises the chosen tracks to temp `.vtt` (`materializeHls()` for Mux HLS renditions via ffmpeg, `materializeDirect()` for Cloudflare/Vimeo direct `.vtt` via Guzzle + optional cookie), then `deliver()` embeds (from the `.vtt`) and/or saves `.srt` sidecars into `subs/` and cleans up the temps. `--subtitles-only` always writes sidecars into `subs/` (regardless of mode), and with no `-s` covers every downloaded series (the `*-only` modes stay on the normal no-filter path in `Downloader::start` — `getSeries($cache, $cacheOnly)`, fresh unless `--cache-only` — then `array_intersect_key` against the on-disk library so undownloaded series get no folders/orphan sidecars); the existing-subs check in `Resolver::downloadEpisodeSubtitles` looks in `subs/` (matches `.srt` and `.vtt`). `commands/MoveSubtitles.php` (`--dry-run`, `-s slug`) migrates an older library's `.vtt` from next-to-the-video into `subs/`; `commands/ConvertSubtitlesToSrt.php` (`--dry-run`, `-s slug`) converts an existing library's `subs/*.vtt` to `.srt` (skips on `.srt` collision, removes the source `.vtt`).
...
- **The filesystem is the download state.** Episodes are saved as `series/<slug>/NN-<sanitized title>.mp4` (`%02d` number; `Utils::parseEpisodeName` keeps the lesson title verbatim except for Windows-illegal chars `\/:*?"<>|` and trailing dots/spaces). The diff in `Utils::compareLocalAndOnlineSeries` compares episode-number prefixes against the online list, and skips a series entirely when local file count equals `episode_count`. Only `.mp4` files count (`.srt`/`.vtt` subtitles and `.part` leftovers are ignored by the scan). Renaming or deleting files triggers re-download. `commands/ReconcileNames.php` (`--dry-run`, `-s slug`) renames an existing library (mp4s, their `.chapters.txt`/`.vtt`/`.srt` sidecars — including those under `chapters/` and `subs/` — and `#<slug>.m3u8` playlist entries) to the current cache.json titles after sanitizer changes.
- **Series playlists.** `App\Utils\Playlist::generate()` writes each series' `#<slug>.m3u8` — the episode mp4s in order, one bare basename per line (the simple format `ReconcileNames` keeps in sync; CRLF, no `#EXTM3U` header). `WRITE_PLAYLIST=true` regenerates it after a series downloads (the hook in `Downloader::downloadEpisodes` is source-agnostic — mux/external/cloudflare), and `--playlist-only` rebuilds it across the local library (`Downloader::generatePlaylists`, no `-s` required; `-e` ignored). Generation scans `BASE_FOLDER/series/<slug>/*.mp4` directly (top-level only, so the `chapters/` and `subs/` subfolders are not scanned).
