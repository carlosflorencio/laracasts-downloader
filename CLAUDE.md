...
# lesson publish dates (no videos); combinable. Like --metadata-only these no
# longer require -s: with no filter they backfill the whole local library.
# Always sidecar-only (chapters -> chapters/, subtitles -> subs/, never
# embedded, regardless of DOWNLOAD_CHAPTERS/DOWNLOAD_SUBTITLES); --timestamps-only
# only touches already-downloaded videos.
php start.php --subtitles-only --chapters-only           # whole local library
