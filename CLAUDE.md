...
- **The filesystem is the download state.** Episodes are saved as `series/<slug>/NN-<sanitized title>.mp4` (`%02d` number; `Utils::parseEpisodeName` keeps the lesson title verbatim except for Windows-illegal chars `\/:*?"<>|` and trailing dots/spaces). The diff in `Utils::compareLocalAndOnlineSeries` compares episode-number prefixes against the online list, and skips a series entirely when local file count equals `episode_count`. Only `.mp4` files count (`.vtt` subtitles and `.part` leftovers are ignored by the scan). Renaming or deleting files triggers re-download. `commands/ReconcileNames.php` (`--dry-run`, `-s slug`) renames an existing library (mp4s, their `.chapters.txt`/`.vtt` sidecars — including those under `chapters/` and `subs/` — and `#<slug>.m3u8` playlist entries) to the current cache.json titles after sanitizer changes.
