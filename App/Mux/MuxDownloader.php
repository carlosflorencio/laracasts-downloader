...
class MuxDownloader
{
    ...
    public function download(string $playbackId, string $token, string $filepath): bool
    {
        ...
        $result = $this->downloadAndMerge($video['url'], $audio['url'] ?? null, $filepath);
     ...
     * mp4 with ffmpeg (stream copy, no re-encoding).
     */
    private function downloadAndMerge(string $videoURL, ?string $audioURL, string $outputPath): bool
    {
        $inputs = $audioURL === null
            ? sprintf('-i %s', escapeshellarg($videoURL))
            : sprintf('-i %s -i %s -map 0:v:0 -map 1:a:0', escapeshellarg($videoURL), escapeshellarg($audioURL));

        $command = sprintf('ffmpeg -y -hide_banner -loglevel error %s -c copy %s 2>&1', $inputs, escapeshellarg($outputPath));
